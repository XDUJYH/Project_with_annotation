...
public final class TicketAvailabilityTokenBucket {
    ...
    public boolean takeTokenFromBucket(PurchaseTicketReqDTO requestParam) {
        ...
        //这段代码使用了Spring Data Redis库中的DefaultRedisScript类，该类用于执行Lua脚本。具体而言，它涉及到获取或创建一个Lua脚本对象，并将其赋值给DefaultRedisScript<Long>类型的变量actual。
        //Singleton.get(...)：
        //这里使用了某种单例模式的实现，通过Singleton.get方法获取Lua脚本对象。在这个方法的实现中，如果已存在Lua脚本对象，则直接返回；如果不存在，则创建一个新的Lua脚本对象。这种单例模式的实现可能是为了避免重复创建Lua脚本对象，以提高性能。
        //总体而言，这段代码的目的是获取或创建一个Lua脚本对象，以便后续可以在Redis中执行相应的Lua脚本。这通常用于在Redis中执行一些原子操作或复杂的逻辑，因为Lua脚本可以在Redis服务器端执行，减少了多次网络往返的开销。
